...
$getquery = mysqli_query($conDB, "SELECT * FROM `customer` WHERE `id`='".$_GET['id']."' ");
	...
	if($card_exp_up){
		$query="INSERT INTO `cust_card_update` (`cust_no`,`injazat_no`, `exp_date`,`status`, `date_reg`, `sectin_nme`) VALUES ('".$id_cust."', '".$injazat_no."', '".$card_exp_up."', '".$status_up."', '".$date_reg_up."', '".$sectin_nme_up."')";
		mysqli_query($conDB, $query);

        $u = "UPDATE `cust_card_update` SET `status`='I' WHERE `id`='".$upd_status_lst."' ";
        mysqli_query($conDB, $u) or die (mysqli_error());

        $cust_u = "UPDATE `customer` SET `exp_date`='".$card_exp_up."' WHERE `id`='".$_GET['id']."' ";
						...
						<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
							...
							<div class="form-row">
								<div class="form-group col-md-4">
                                    ...
                                    <input type="text" name="card_exp" parsley-trigger="change" class="form-control" id="card_exp">
								</div>
								<div class="form-group col-md-4">
									<label for="sectin_nme" class="col-form-label">Received from Shop</label>
