<?php 	
	...
	$id=$_POST['id'];
    $msql = "DELETE FROM `smart_request` WHERE `inv_no` = '".$_POST['id']."'";
    if (mysqli_query($conDB, $msql)) {
		mysqli_query($conDB, "DELETE FROM `smt_request_status` WHERE `inv_no` = '".$_POST['id']."' ");
		$sql=mysqli_query($conDB, "SELECT * FROM `smt_attachment` WHERE `inv_no` = '".$_POST['id']."' ");
			while($row = mysqli_fetch_array($sql)){
				$attachment = $row['attachment'];
				if(unlink("../assets/assets/smt_attachment/".$attachment));
			}
		mysqli_query($conDB, "DELETE FROM `smt_attachment` WHERE `inv_no` = '".$_POST['id']."' ");
        $data = [
    		'title'   => "Deleted!",
