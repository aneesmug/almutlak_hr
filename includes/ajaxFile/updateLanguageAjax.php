...
try {
    // 1. Update the English translation only (keep the original lang_key).
    $sql_en = "UPDATE translations SET translation = '{$en_translation}' WHERE lang_key = '{$original_lang_key}' AND lang_code = 'en'";
    if (!mysqli_query($conDB, $sql_en)) {
        throw new Exception("Error updating English translation.");
    }

    // 2. Update the Arabic translation only (keep the original lang_key).
    $sql_ar = "UPDATE translations SET translation = '{$ar_translation}' WHERE lang_key = '{$original_lang_key}' AND lang_code = 'ar'";
    if (!mysqli_query($conDB, $sql_ar)) {
        // If the Arabic record doesn't exist for some reason, create it.
        if (mysqli_affected_rows($conDB) == 0) {
            $sql_ar_insert = "INSERT INTO translations (lang_key, lang_code, translation) VALUES ('{$original_lang_key}', 'ar', '{$ar_translation}')";
