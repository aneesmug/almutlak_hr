...
function getColumnLabel($column) {
    ...
    $labels = [
        ...
        'hours_benefit' => 'Hours Benefits',
        'days_benefit' => 'Days Benefits',
        'other_income_benefit' => 'Other Income',
...
function generatePayrollReport($conDB, $columns, $departments, $dateFrom, $dateTo, $hasFullAccess, $userDept, $status = '', $employeeId = '', $companies = [], $countries = []) {
    ...
    foreach ($columns as $col) {
        switch ($col) {
            ...
            case 'total_benefits':
                ...
                break;
            case 'hours_benefit':
                $selectCols[] = "(SELECT COALESCE(SUM(CAST(pb.note AS DECIMAL(10,2))), 0) FROM payroll_benefits pb WHERE pb.emp_id = p.emp_id AND pb.month = p.month_year AND pb.status = 1 AND (COALESCE(pb.hours, 0) > 0 OR pb.calculation_type IN ('overtime_basic', 'overtime_total'))) AS hours_benefit";
                break;
            case 'days_benefit':
                $selectCols[] = "(SELECT COALESCE(SUM(CAST(pb.note AS DECIMAL(10,2))), 0) FROM payroll_benefits pb WHERE pb.emp_id = p.emp_id AND pb.month = p.month_year AND pb.status = 1 AND COALESCE(pb.days, 0) > 0 AND COALESCE(pb.hours, 0) = 0 AND pb.calculation_type NOT IN ('overtime_basic', 'overtime_total')) AS days_benefit";
                break;
            case 'other_income_benefit':
                $selectCols[] = "(SELECT COALESCE(SUM(CAST(pb.note AS DECIMAL(10,2))), 0) FROM payroll_benefits pb WHERE pb.emp_id = p.emp_id AND pb.month = p.month_year AND pb.status = 1 AND COALESCE(pb.hours, 0) = 0 AND COALESCE(pb.days, 0) = 0 AND pb.calculation_type NOT IN ('overtime_basic', 'overtime_total')) AS other_income_benefit";
                break;
    ...
    if (empty($selectCols)) {
        $selectCols = [
            ...
            "(SELECT COALESCE(SUM(CAST(pb.note AS DECIMAL(10,2))), 0) FROM payroll_benefits pb WHERE pb.emp_id = p.emp_id AND pb.month = p.month_year AND pb.status = 1 AND (COALESCE(pb.hours, 0) > 0 OR pb.calculation_type IN ('overtime_basic', 'overtime_total'))) AS hours_benefit",
    ...
    // Build WHERE clause. NOTE: no employee-active-status filter here, matching
    // get_payroll_report.php (the Payroll Summary Report source of truth) - a payroll
    // row that was already generated/paid for the month should keep showing even if the
    // employee was later deactivated.
    $where = [];
    ...
    // Exclude Hold-payment employees, matching the Payroll Summary Report
    // (get_payroll_report.php) and Total Net figures used elsewhere.
    $where[] = "e.payment_type <> 3";

    // Vacation dropout filter - exclude employees on a long vacation covering the
    // payroll month, same rule process_payroll.php/get_payroll_report.php use to skip
    // payroll generation for that month.
    ...
    if ($vacationDropoutDays > 0) {
        $where[] = "NOT EXISTS (
                ...
                WHERE vd.emp_id = e.emp_id
                    ...
                    AND vd.start_date <= LAST_DAY(CONCAT(p.month_year, '-01'))
                    AND COALESCE(vd.return_date, vd.start_date) >= CONCAT(p.month_year, '-01')
                    AND COALESCE(vd.vacdays, 0) >= {$vacationDropoutDays}
                    AND vd.request_inv_no LIKE 'VAC-%'
            )";
    }

    // Paid vacation settlement filter - exclude employees already settled for the
    // period covering this payroll month, mirroring get_payroll_report.php.
    $where[] = "NOT EXISTS (
            ...
            WHERE sr.emp_id = e.emp_id
                ...
                AND (
                        (
                            ...
                            AND sv.start_date <= LAST_DAY(CONCAT(p.month_year, '-01'))
                            AND COALESCE(sv.return_date, sv.start_date) >= CONCAT(p.month_year, '-01')
                            AND (
                                sv.return_date IS NULL
                                OR sv.return_date > LAST_DAY(CONCAT(p.month_year, '-01'))
                            )
                        )
                        OR
                        (
                            sv.id IS NULL
                            AND sr.payment_date BETWEEN CONCAT(p.month_year, '-01') AND LAST_DAY(CONCAT(p.month_year, '-01'))
                        )
                )
        )";
