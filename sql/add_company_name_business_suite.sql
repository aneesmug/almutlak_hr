...
-- Arabic company name (used in the header title when the language is Arabic; falls back to auto-translation if empty).
INSERT INTO `app_settings` (`setting_name`, `setting_value`, `setting_group`, `description`, `input_type`, `options`)
SELECT 'company_name_ar', 'المطلق', 'general', 'Company name (Arabic)', 'text', NULL
FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM `app_settings` WHERE `setting_name` = 'company_name_ar');
